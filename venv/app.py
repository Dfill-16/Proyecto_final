...
# Crear el <select> para los países únicos
    ...
    todo=todo+"<div class='col-md-2'><button onclick='mostrarGrafico()' class='mt-2 p-3 bg-green-500 text-black rounded-lg shadow-md transition duration-700 ease-in-out hover:bg-blue-600 hover:text-white'>Graficar</button></div>"
    todo=todo+"</div>"
...
def graficar(pais, grafico):
        ...
        graph_html = fig.to_html(full_html=False)+"<br><button onclick='cargarData()' class='p-3 rounded-lg shadow-md transition duration-300 ease-in-out hover:bg-green-400'>Regresar</button>"
        return graph_html    

@app.route('/image/<img_id>')
def serve_image(img_id): 
    image_path = f'static/images/foto{img_id}.avif' 
